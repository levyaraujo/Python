...
2. Calculates the fibonacci sequence with a formula
    ...
    reference-->Su, Francis E., et al. "Fibonacci Number Formula."
...
def _check_number_input(n, min_thresh, max_thresh=None):
    ...
    try:
        ...
        print(
            f"""Incorrect Input: input number must be > {min_thresh} 
            for the recursive calculation"""
        )
    except ValueTooLargeError:
        print(
            f"""Incorrect Input: input number must be < {max_thresh} 
            for the recursive calculation"""
        )
    return False
